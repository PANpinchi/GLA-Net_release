...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, layer_idx=0):
...
class GLANet(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for layer, conv_layer in zip(self.layers, self.conv_layers):
            x = x_short

            pre_feature = x  # (B, current_dim, C)

            # Change to 2D
            x = x.view(B, H, W, C * i_layer)  # (B, H, W, C * i_layer)
            x = x.permute(0, 3, 1, 2).contiguous()  # (B, C * i_layer, H, W)

            x = conv_layer(x)  # (B, current_dim, H, W) -> (B, embed_dim, H, W)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x_ = torch.nn.functional.interpolate(x, scale_factor=self.upscale, mode='bilinear')
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x)) + x
            x = self.conv_before_upsample(x)
            x = self.conv_last(self.upsample(x)) + x_
...
if __name__ == '__main__':
    ...
    model = GLANet(upscale=model_opt['upscale'],
                   ...
                   img_range=model_opt['img_range'],
                   depths=model_opt['depths'],
                   embed_dim=model_opt['embed_dim'],
                   num_heads=model_opt['num_heads'],
                   mlp_ratio=model_opt['mlp_ratio'],
                   upsampler=model_opt['upsampler'],
                   resi_connection=model_opt['resi_connection'])
